...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if ask_to_run "5.3 消融实验 - 无正交投影压力测试 (ablation_obj_stress.py)"; then
        run_python "ablation_obj_stress.py" "消融实验 - 无正交投影压力测试"
    fi
    if ask_to_run "5.4 消融实验 - 语义绑定 (ablation_sem.py)"; then
        run_python "ablation_sem.py" "消融实验 - 语义绑定"
    fi
    if ask_to_run "5.5 消融实验 - 网格分辨率 (ablation_grid.py)"; then
        run_python "ablation_grid.py" "消融实验 - 网格分辨率"
    fi
    if ask_to_run "5.6 消融实验 - 定量分析 (analyze_ablation.py)"; then
        run_python "analyze_ablation.py" "消融实验 - 定量分析"
    fi
    if ask_to_run "5.7 消融实验 - 可视化对比 (visualize_ablation.py)"; then
        run_python "visualize_ablation.py" "消融实验 - 可视化对比"
    fi
fi

# ==========================================
# Phase 6: 消融攻击一体化流水线（可选，用于快速单图验证）
# ==========================================
echo "##########################################"
echo "# Phase 6: 消融攻击+提取一体化流水线（可选）"
echo "##########################################"
echo ""

read -p "是否运行消融攻击一体化流水线 (ablation_attack_pipeline.py)? (y/n) " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "提示: 此脚本需要指定输入目录和输出目录，例如:"
    echo "  python3 ablation_attack_pipeline.py --input_dir ./pic/watermarked_img --output_dir ./ablation_pipeline_result"
    echo "请手动运行上述命令"
fi
...
echo "│   ├── ablation_obj_stress_watermarked_img/ # 消融实验：无正交压力测试"
echo "│   ├── ablation_sem_watermarked_img/        # 消融实验：语义绑定"
echo "│   └── result/ablation_grid/                # 消融实验：网格分辨率"
echo "│"
