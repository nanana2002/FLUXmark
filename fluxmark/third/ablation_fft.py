#!/usr/bin/env python3
"""
消融实验：不加FFT约束的水印生成（极致性能版 - 50GB显存全速运行）
对比正常版本，验证FFT带通滤波的重要性
...
output_dir = os.path.join(config['output_base_dir'], 'pic', 'ablation_fft_watermarked_img')
...
# 5. 批量生成消融实验图像（高性能模式）
# ==========================================
print(f"\n🎨 开始生成 {len(prompts)} 张消融实验图像（无FFT）...\n")

results_summary = {
    'experiment_name': f"{config['experiment_name']}_ablation_fft",
    'timestamp': datetime.now().isoformat(),
    'ablation_type': 'no_fft_constraint',
...
for batch_start in tqdm(range(0, len(prompts), batch_size), desc="生成批次"):
    ...
    for idx, img_idx in enumerate(batch_indices):
        img_id = f"ablation_fft_{img_idx:03d}"
        ...
        with torch.no_grad():
            ...
            v_pred = pipe.transformer(
                ...
                pooled_projections=batch_pooled_embeds[idx:idx+1],
                encoder_hidden_states=batch_prompt_embeds[idx:idx+1],
                txt_ids=batch_text_ids[idx:idx+1],
                img_ids=img_ids,
                return_dict=False,
            )[0]
...
# 6. 保存摘要
# ==========================================
summary_path = os.path.join(output_dir, f'{config["experiment_name"]}_ablation_fft_summary.json')
