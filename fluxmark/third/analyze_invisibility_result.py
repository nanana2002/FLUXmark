...
if FID_AVAILABLE:
    ...
    # 使用全部可用样本
    num_samples = min(len(no_wm_summary['images']), len(watermark_summary['images']))
    ...
    print("   加载无水印图像...")
    images_real = load_images_batch(real_paths)

    print("   加载水印图像...")
    images_fake = load_images_batch(fake_paths)
