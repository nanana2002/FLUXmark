...
mask = ((radius >= 4) & (radius <= 12)).float().unsqueeze(0).unsqueeze(-1) 
...
# 🌟 新增绝杀：Token 级能量均衡！
# 强行抹平 FFT 带来的空间能量涟漪，确保每一个 token (也就是每一个像素块) 的初始水印强度绝对一致！
...
def orthogonal_drift_callback(pipe, step_index, timestep, callback_kwargs):
    ...
    # 🌟 核心修正：使用 dim=-1 进行 Token 级正交化！
    # 以前是跨越所有 1024 个 token 的 sum，现在我们在每一个 token 内部（64维）做独立的投影。
    # 这样，猫耳朵的语义绝对不会干扰猫鼻子的水印！
    ...
    # Token 级的能量重归一化，保证每个局部的注入强度精准一致
