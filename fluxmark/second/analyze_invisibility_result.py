...
def compute_fid(images_real, images_fake, device='cuda'):
    ...
    # 分批计算（避免OOM）
    batch_size = 10  # 减小batch size适配22GB显存
...
def compute_clip_score(images, prompts, device='cuda'):
    """
    计算CLIP Score (图文语义一致性)

    Args:
        images: 图像批次 [N, 3, H, W], 值域 [0, 1]
        prompts: prompt列表

    Returns:
        clip_score: CLIP分数，越高越好
    """
    ...
    # 分批计算
    scores = []
    batch_size = 5  # 减小batch size适配22GB显存
